...
static MCBSP_Config mcbspforAIC23Cfg = {
        ...
        MCBSP_FMKS(SPCR, FRST, YES)             |	// Frame-sync generator reset bit *** changed to YES % FSXM is derived from SRG, so the reset should be generated by SRG
        ...
        MCBSP_FMKS(SPCR, XRST, YES)             |	// Transmitter reset bit resets or enables the transmitter. ***changed to YES to
...
void Config_DSK6713_AIC23(void)
{
    ...
    MCBSP_config(hMcbsp0, &mcbspforAIC23Cfg); /* changed at &mcbspforAIC23Cfg because the configuration of Mcbsp0 is stored at mcbspforAIC23Cfg{} */
    MCBSP_start(hMcbsp0,  MCBSP_XMIT_START | MCBSP_RCV_START, 0xffffffff); //
...
static void set_aic23_register(MCBSP_Handle hMcbsp,unsigned short regnum, unsigned short regval)
{
    ...
    /* warten */
    while (!MCBSP_xrdy(hMcbsp)); // function return XRDY which is set to 1 if
    
    /* schreiben */
    //MCBSP_write(hMcbsp, 0x00); // write NULL
}
